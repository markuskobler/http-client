...
mod tcp;
#[cfg(any(feature = "native-tls", feature = "rustls_client"))]
mod tls;
...
impl Debug for H1Client {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let https_pools = if cfg!(any(feature = "native-tls", feature = "rustls_client")) {
            self.http_pools
                .iter()
                .map(|pool| {
...
impl HttpClient for H1Client {
    async fn send(&self, mut req: Request) -> Result<Response, Error> {
        ...
        if scheme != "http"
            && (scheme != "https" || cfg!(not(any(feature = "native-tls", feature = "rustls_client"))))
        {
